# mu-ippt — AI 生成原生可编辑 PPTX，支持任意文档输入
...
<p align="center">
  <a href="https://mupoet.github.io/mu-ippt/"><strong>在线预览</strong></a> ·
...
> **官方渠道 —** 本项目**仅**在 [GitHub](https://github.com/mupoet/mu-ippt)（主仓库）和 [AtomGit](https://atomgit.com/mupoet/mu-ippt)（自动同步镜像）发布。其他平台上的转发版本均为非官方版本，不由作者维护。遵循 MIT 协议——使用需保留署名。

---

丢进一份 PDF、DOCX、网址或 Markdown，拿回一份**原生可编辑的 PowerPoint**——真正的形状、真正的文本框、真正的图表，不是图片。点击任何元素即可编辑。

**[为什么选 mu-ippt？](./docs/zh/why-mu-ippt.md)**
...
- **真正的 PPT** — 如果一个文件在 PowerPoint 里打不开、不能编辑，它就不应该被叫做 PPT。mu-ippt 输出的每个元素都能直接点击修改
...
**[在线预览 →](https://mupoet.github.io/mu-ippt/)** · [`examples/`](./examples/) — 15 个项目，229 页
...
mu-ippt 源于一个真实的痛点：在投融资和咨询工作中，我每天都要制作和审阅大量 PPT，而市面上的 AI 幻灯片工具导出的都是图片，不是可编辑的元素。作为一个每天都需要点击进去修改内容的人，这完全不可接受。我需要的是真正的 DrawingML——点击任何元素都能直接编辑，就像手工搭建的一样。
...
[GitHub](https://github.com/mupoet/mu-ippt) → **Code → Download ZIP** · [AtomGit](https://atomgit.com/mupoet/mu-ippt) → **克隆/下载 → 下载ZIP**（国内网速更快）
...
# AtomGit（国内网速更快）
git clone https://atomgit.com/mupoet/mu-ippt.git
cd mu-ippt
```
...
日常更新（仅方式 B）：`python3 skills/mu-ippt/scripts/update_repo.py`
...
> **AI 迷失上下文？** 让它先读 `skills/mu-ippt/SKILL.md`。
...
运行 `python3 skills/mu-ippt/scripts/image_gen.py --list-backends` 查看分级。环境变量优先于 `.env`。使用各家独立的 Key（`GEMINI_API_KEY`、`OPENAI_API_KEY` 等）——不支持全局 `IMAGE_API_KEY`。
...
| | 文档 | 说明 |
|---|------|------|
| 🆚 | [为什么选 mu-ippt](./docs/zh/why-mu-ippt.md) | 与 Gamma、Copilot 等工具的对比 |
| 🪟 | [Windows 安装指南](./docs/zh/windows-installation.md) | Windows 用户手把手安装教程 |
| 📖 | [SKILL.md](./skills/mu-ippt/SKILL.md) | 核心流程与规则 |
| 📐 | [画布格式](./skills/mu-ippt/references/canvas-formats.md) | PPT 16:9、小红书、朋友圈等 10+ 种格式 |
| 🛠️ | [脚本与工具](./skills/mu-ippt/scripts/README.md) | 所有脚本和命令 |
...
欢迎合作交流、将 mu-ippt 集成到你的工作流，或者单纯提问：
...
[⬆ 回到顶部](#mu-ippt--ai-生成原生可编辑-pptx支持任意文档输入)
