...
Dispatches to configurable image backends (OpenAI, Gemini).
...
Supported keys:
  IMAGE_BACKEND      (optional) backend name, defaults to "openai"
...
def _resolve_backend() -> tuple[object, str]:
    ...
    Defaults to 'openai' when IMAGE_BACKEND is not set.

    Returns:
        A backend module with a generate() function.
    """
    backend_name = os.environ.get("IMAGE_BACKEND", "openai").strip().lower()
