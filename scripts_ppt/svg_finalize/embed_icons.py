...
_ICONS_BUNDLE = DEFAULT_ICONS_DIR / 'icons-bundle.tar.gz'


def _ensure_icons_extracted():
    """Auto-extract icon bundle on first run if directories are missing."""
    if not _ICONS_BUNDLE.exists():
        return
    # Check if any library dir is missing
    needed = ['chunk', 'tabler-filled', 'tabler-outline']
    if all((DEFAULT_ICONS_DIR / d).is_dir() for d in needed):
        return
    print(f"[icons] Extracting icon bundle ({_ICONS_BUNDLE.name})...")
    with tarfile.open(_ICONS_BUNDLE, 'r:gz') as tar:
        tar.extractall(path=DEFAULT_ICONS_DIR)
    print(f"[icons] Done — {sum(1 for d in needed if (DEFAULT_ICONS_DIR / d).is_dir())}/3 libraries ready")
