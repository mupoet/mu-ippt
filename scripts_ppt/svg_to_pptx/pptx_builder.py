...
def create_pptx_with_native_svg(
    ...
    # Auto-detect SVG viewBox mismatch: if the SVG viewBox differs from the
    # target slide dimensions, use the SVG's viewBox as the actual slide size.
    # This ensures native DrawingML shapes (whose EMU coordinates are derived
    # directly from SVG px values) fill the slide correctly.
    if use_native_shapes and svg_files:
        svg_vb = get_viewbox_dimensions(svg_files[0])
        if svg_vb and (svg_vb[0] != pixel_width or svg_vb[1] != pixel_height):
            if verbose:
                print(f"  ⚠ SVG viewBox ({svg_vb[0]}×{svg_vb[1]}) != target slide ({pixel_width}×{pixel_height})")
                print(f"    → Adjusting slide dimensions to match SVG viewBox")
            pixel_width, pixel_height = svg_vb
            width_emu, height_emu = get_slide_dimensions(None, svg_vb)
            # Keep the same aspect ratio check for 16:9
            vb_ratio = svg_vb[0] / svg_vb[1]
            target_ratio = 16 / 9
            if abs(vb_ratio - target_ratio) > 0.01 and verbose:
                print(f"    ⚠ SVG aspect ratio ({vb_ratio:.3f}) differs from 16:9 ({target_ratio:.3f})")
