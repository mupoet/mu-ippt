...
```bash
python3 ${SKILL_DIR}/scripts_ppt/pptx_template_import.py "<reference_template.pptx>"
```
...
```bash
ls -la "${SKILL_DIR}/templates/layouts/<template_id>"
```

Run SVG validation on the template directory:

```bash
python3 ${SKILL_DIR}/scripts_ppt/svg_quality_checker.py "${SKILL_DIR}/templates/layouts/<template_id>" --format <canvas_format>
```
...
Update `${SKILL_DIR}/templates/layouts/layouts_index.json`:
...
**Template Path**: `${SKILL_DIR}/templates/layouts/<template_id>/`
**Category**: <category>
